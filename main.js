...
function injectProfilePhoto() {
  ...
  if (el) el.src = PROFILE_PHOTO;
}

/* ===== PROJECT IMAGES — reads PROJECT_IMAGES from config.js ===== */
function injectProjectImages() {
  // Covers both home work cards (.wc) and projects page cards (.pc)
  document.querySelectorAll('.wc[onclick], .pc[onclick]').forEach(card => {
    // Extract project ID from onclick="nav('detail','scryai')"
    const match = card.getAttribute('onclick').match(/nav\('detail','(\w+)'\)/);
    if (!match) return;
    const id = match[1];
    const url = (PROJECT_IMAGES && PROJECT_IMAGES[id]) ? PROJECT_IMAGES[id].trim() : '';
    if (!url) return; // no image set — keep wireframe placeholder

    // Find the inner image container
    const inner = card.querySelector('.wi-inner, .pimg-in');
    if (!inner) return;

    // Create <img> and layer it on top of the wireframe background
    const img = document.createElement('img');
    img.src = url;
    img.alt = id + ' project screenshot';
    img.loading = 'lazy';
    img.style.cssText = 'position:absolute;inset:0;width:100%;height:100%;object-fit:cover;display:block;border-radius:0;';

    // If image fails to load, remove it so wireframe shows instead
...
function nav(page, sub = '') {
  ...
  setTimeout(() => {
    ...
    if (page === 'home') { setTimeout(() => { initCanvas(); initCounters(); buildGallery('gallery-home'); injectProjectImages(); }, 80); }
    if (page === 'projects') { setTimeout(() => { injectProjectImages(); }, 80); }
...
function loadDetail(id) {
  ...
  // Inject real project image at the top of detail body if available
  const imgUrl = (PROJECT_IMAGES && PROJECT_IMAGES[id]) ? PROJECT_IMAGES[id].trim() : '';
  const heroImg = imgUrl
    ? `<div style="width:100%;border-radius:1.25rem;overflow:hidden;margin-bottom:3rem;border:1px solid var(--border)">
        <img src="${imgUrl}" alt="${id} project" style="width:100%;display:block;object-fit:cover;"
          onerror="this.parentElement.style.display='none'" />
       </div>`
    : '';

  document.getElementById('d-body').innerHTML = heroImg + p.body + behLink;
}
...
function makePlaceholder(ph) {
  return `<div class="g-ph" style="width:100%;height:100%;background:${ph.bg};display:flex;align-items:center;justify-content:center;flex-direction:column;gap:6px">
...
function makeItem(idx) {
  ...
  if (imgUrl && imgUrl.trim()) {
    a.innerHTML = `<img src="${imgUrl.trim()}" alt="Photography by ${INFO.instaHandle}" loading="lazy" />
      <div class="g-ovr"><svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="white" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"><rect x="2" y="2" width="20" height="20" rx="5"/><circle cx="12" cy="12" r="4"/><circle cx="17.5" cy="6.5" r="0.5" fill="white" stroke="none"/></svg></div>`;
    // fallback to placeholder if image fails
    a.querySelector('img').onerror = function() {
      a.innerHTML = makePlaceholder(ph);
    };
  } else {
    a.innerHTML = makePlaceholder(ph);
  }
